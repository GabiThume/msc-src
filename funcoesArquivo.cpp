...
int descriptor(char const *baseImagem, int method, int nColor, double nRes, int oNorm, int *param, int nparam, int oZero, int quantMethod)
{
    ...
    char *diretorioDescritores = (char *)"caracteristicas";
	char *quantMethodsNames[4] = {(char *)"Intensity", (char *)"Gleam", (char *)"Luminance", (char *)"MSB"};
	
	switch (method) {
	case 1: 
	    sprintf(nome,"%s/%s_BIC_%s_%dc_%dr.txt", diretorioDescritores, baseImagem, quantMethodsNames[quantMethod-1], nColor, rfactor);
	    fvsize = nColor*2;
	    cout << "BIC and " << quantMethodsNames[quantMethod-1] << ":";
	    break;
	case 2:
	    sprintf(nome,"%s/%s_GCH_%s_%dc_%dr.txt", diretorioDescritores, baseImagem, quantMethodsNames[quantMethod-1], nColor, rfactor);
	    fvsize = nColor;
	    cout << "GCH and " << quantMethodsNames[quantMethod-1] << ":";
	    break;
	case 3:
	    sprintf(nome,"%s/%s_CCV_%s_%dc_%dr.txt", diretorioDescritores, baseImagem, quantMethodsNames[quantMethod-1],  nColor, rfactor);
	    fvsize = nColor*2;
	    cout << "CCV and " << quantMethodsNames[quantMethod-1] << ":";
	    break;
	case 4:
	    sprintf(nome,"%s/%s_Haralick6_%s_%dc_%dr.txt", diretorioDescritores, baseImagem, quantMethodsNames[quantMethod-1],  nColor, rfactor);
	    ...
	    break;
	case 5:
	    sprintf(nome,"%s/%s_ACC_%s_%dc_%dd_%dr.txt",  diretorioDescritores, baseImagem, quantMethodsNames[quantMethod-1], nColor, nparam, rfactor);
