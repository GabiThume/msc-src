...
class Artificial {
  ...
  public:
    std::string generate(std::string base, std::string newDirectory,
      int whichOperation);
    void GenerateImage(std::vector<cv::Mat> images, std::string name, int total,
      int generationType);
    ...
    cv::Mat generateComposition(cv::Mat originalImage,
