...
void classifica(string dir, string features, string csvName, pair<int, int> min, int d, int m, int operation){
    ...
    string fileDescriptor = desc(dir, features, d, m, numImg);

    data = readFeatures(fileDescriptor, &classes, &numClasses);
    size = data.size();
    if (size.height != 0){
    ...
    */
    for (d = 1; d <= 1; d++){
        ...
        for (m = initialMethod; m <= endMethod; m++){
            ...
            string fileDescriptor = desc(baseDir, featuresDir, d, m, "");
