...
class plot(object):
    ...
    def __init__(self, csvName, label_dict):
        ...
        df = pd.io.parsers.read_csv(filepath_or_buffer = self.csvName, header=None, sep=',', lineterminator='\n')
        df.dropna(how="all", inplace=True, axis=1)
        df.columns = ['Image path'] + ['Class label'] + ['Fold'] + ['Not set(0) Train(1) Test(1)'] + ['Original(0) Smote(1) Generated(2)'] + [l for l in range(5, df.shape[1])]
        self.all_samples = df[range(5, df.shape[1])].values

        self.all_classes = df['Class label'].values
        self.num_classes = len(np.unique(self.all_classes))
        self.images = df['Image path'].values

        trainOrTest = df['Not set(0) Train(1) Test(2)'].values
        self.treino = trainOrTest == 1
        if self.treino.sum() == 0:
            self.treino = trainOrTest == 0
        self.teste = trainOrTest == 2

        self.generated = df['Original(0) Smote(1) Generated(2)'].values == 2
        self.smote = df['Original(0) Smote(1) Generated(2)'].values == 1

        # Take only the original training samples
        self.original = (df['Original(0) Smote(1) Generated(2)'].values == 0) * self.treino
        ...
        for label in range(self.num_classes):
            self.samples.append([
                (self.all_classes == label) * self.treino * self.original,
...
csvName = "/media/gabi/Dados 2/data/experiments/datasets/praia-montanha/artificial/experiment-0/features/smote_ACC_Gleam_64_256.csv"
